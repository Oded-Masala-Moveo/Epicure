...
const RestaurantsPage: React.FC = () => {
  ...
  const handelClickRange = (rangeType: string) => () =>
    rangeType == currentRange
      ? setCurrentRange("")
      : setCurrentRange(rangeType);
  const dropdownOpenOrClose = (rangeName: string) => (rangeState: string) =>
    rangeName == rangeState;
  const setRestaurantsData = () => {
    let condition = values[0] > 12 || values[1] < 357 || rating[0];
    if (condition)
      setDisplayRestaurants(
        filterRestaurants(restaurants, selectedCategory, rating, values)
      );

    if (restaurants && !condition)
  ...
  useEffect(() => {
    setRestaurantsData();
  }, [restaurants,selectedCategory, rating, values]);
  return (
    <>
      <section onClick={sendCloseNavbar()} className="restaurants-section">
        ...
        <ul className="range-filter-restaurant">
          <div
            className={currentRange == RestaurantRange.PRICE ? "selected" : ""}
          >
            <Dropdown
              key={RestaurantRange.PRICE}
              onClick={handelClickRange(RestaurantRange.PRICE)}
              isOpen={dropdownOpenOrClose(RestaurantRange.PRICE)(currentRange)}
              dropdownLook={<LiElement title={RestaurantRange.PRICE} />}
              children={
                <PriceComponent values={values} setValues={handleChange} />
              }
            />
          </div>
          <div className="disable">
            <Dropdown
              key={RestaurantRange.DISTANCE}
              onClick={handelClickRange(RestaurantRange.DISTANCE)}
              isOpen={dropdownOpenOrClose(RestaurantRange.DISTANCE)(
                currentRange
              )}
              dropdownLook={<LiElement title={RestaurantRange.DISTANCE} />}
              children={<DistanceComponent />}
            />
          </div>
          <div
            className={currentRange == RestaurantRange.RATING ? "selected" : ""}
          >
            <Dropdown
              key={RestaurantRange.RATING}
              onClick={handelClickRange(RestaurantRange.RATING)}
              isOpen={dropdownOpenOrClose(RestaurantRange.RATING)(currentRange)}
              dropdownLook={<LiElement title={RestaurantRange.RATING} />}
              children={
                <RatingComponent rateArr={rating} changeRate={seRrating} />
              }
            />
          </div>
        </ul>
        {displayRestaurants.length > 0 && (
          <div className="restaurants-list" onClick={handelClickRange("")}>
            {displayRestaurants.map((restaurant) => (
              <Card
                key={restaurant.chef + restaurant.image}
                restPage={width < tablet}
                card={restaurant}
              />
            ))}
          </div>
